...
FX=1367.1861572265625  
FY=1366.46875
CX=973.2076416015625  
CY=549.0922241210938
...
def get_template_grasp_point(template_contour, class_name):
    """返回模板坐标系下的抓取点，使用包围盒比例偏移"""
    template_contour = template_contour.reshape(-1, 2).astype(np.float32)
    min_xy = template_contour.min(axis=0)
    max_xy = template_contour.max(axis=0)
    size = max_xy - min_xy

    if np.any(size <= 0):
        return contour_centroid(template_contour)

    if class_name == "L":
        return np.array([min_xy[0] + size[0] * 0.25, min_xy[1] + size[1] * (5.0 / 6.0)], dtype=np.float32)

    if class_name == "l":
        return np.array([min_xy[0] + size[0] * 0.75, min_xy[1] + size[1] * (5.0 / 6.0)], dtype=np.float32)

    if class_name == "T":
        return np.array([min_xy[0] + size[0] * 0.5, min_xy[1] + size[1] * 0.75], dtype=np.float32)

    return contour_centroid(template_contour).astype(np.float32)
...
def match_template(detected_contour, class_name):
    ...
    # 计算抓取点：使用包围盒比例偏移
    template_grasp_point = get_template_grasp_point(template_resampled, class_name)
    grasp_offset = template_grasp_point - template_center
    rotated_grasp_offset = rotate_points(grasp_offset.reshape(1, 2), best_angle, center=(0, 0)).reshape(2,)
    final_grasp_point = rotated_grasp_offset + target_center
